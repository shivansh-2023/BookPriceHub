...
class OpenAIScraper(BaseScraper):
    """Scraper using OpenAI API 4.0 for book information"""
    
    def __init__(self):
        super().__init__()
        self.name = "OpenAI Books"
        self.api_key = os.getenv("OPENAI_API_KEY", "")
        self.base_url = "https://api.openai.com/v1/chat/completions"  # OpenAI v4 endpoint
        self.model = "gpt-4"  # OpenAI v4 model
    ...
    def search(self, book_title):
        """Search for book information using OpenAI API"""
        if not self.api_key:
        ...
        try:
            ...
            self.logger.debug(f"Extracted {len(results)} results from OpenAI API")
            return results
            
        except Exception as e:
            self.logger.error(f"Error searching with OpenAI API: {str(e)}")
            return []
