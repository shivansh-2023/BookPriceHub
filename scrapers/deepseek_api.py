...
class DeepSeekScraper(BaseScraper):
    """Scraper using DeepSeek v3 API for book information"""
    
    def __init__(self):
        super().__init__()
        self.name = "DeepSeek Books"
        self.api_key = os.getenv("DEEPSEEK_API_KEY", "")
        self.base_url = "https://api.deepseek.com/v1/chat/completions"  # DeepSeek v3 endpoint
    ...
    def search(self, book_title):
        """Search for book information using DeepSeek API"""
        if not self.api_key:
            self.logger.error("Cannot search without DeepSeek API key")
            return []
        ...
        try:
            ...
            self.logger.debug(f"Extracted {len(results)} results from DeepSeek API")
            return results
            
        except Exception as e:
            self.logger.error(f"Error searching with DeepSeek API: {str(e)}")
            return []
    
    def _create_messages(self, book_title):
        """Create messages for the DeepSeek API"""
        prompt = f"""
    ...
    def parse_results(self, response):
        ...
        # For the DeepSeek API, we don't use this method from the base class
    ...
    def _parse_response(self, response_json, original_query):
        """Parse the DeepSeek API response into book results"""
        results = []
        
        try:
            # Extract text from response
            if not response_json.get("choices"):
                self.logger.error("No choices in DeepSeek response")
                return results
            ...
            self.logger.error(f"Error parsing DeepSeek response: {str(e)}")
